...
function SetBreakTypeInRedis(obj) {
  try {
    ...
    redisArdsClient.set(breakTypeKey, jsonObj, function (err, result) {
      if (err) {
        logger.error(
          "[DVP-ResResource.SetBreakTypeInRedis] - [REDIS] - SET Failed. [%s] ",
          err
        );
      } else {
        logger.info(
          "[DVP-ResResource.SetBreakTypeInRedis] - [REDIS] - SET Success. [%s] ",
          result
        );
      }
    });
...
function DeleteBreakTypeFromRedis(tenant, company, breakType) {
  try {
    ...
    redisArdsClient.del(breakTypeKey, function (err, result) {
      if (err) {
        logger.error(
          "[DVP-ResResource.RemoveBreakTypeInRedis] - [REDIS] - DEL Failed. [%s] ",
          err
        );
      } else {
        logger.info(
          "[DVP-ResResource.RemoveBreakTypeInRedis] - [REDIS] - DEL Success. [%s] ",
          result
        );
      }
    });
...
function CreateBreakType(
  ...
  DbConn.ResResourceBreakTypes.create(tmpBreakType)
    .then(function (bType) {
      ...
      logger.info(
        "[DVP-ResResource.CreateBreakType] - [PGSQL] - inserted successfully. [%s] ",
        jsonString
      );
    ...
    .error(function (err) {
      logger.error(
        "[DVP-ResResource.CreateBreakType] - [%s] - [PGSQL] - insertion  failed-[%s]",
        breakType,
        err
      );
      jsonString = messageFormatter.FormatMessage(
...
function EditBreakTypeStatus(
  ...
  DbConn.ResResourceBreakTypes.update(
    {
      Active: isActive,
      MaxDurationPerDay: maxDuration,
    },
    ...
    .then(function (bType) {
      ...
      logger.info(
        "[DVP-ResResource.EditBreakTypeStatus] - [PGSQL] - update successfully. [%s] ",
        jsonString
      );
      callback.end(jsonString);
    })
    .error(function (err) {
      logger.error(
        "[DVP-ResResource.EditBreakTypeStatus] - [%s] - [PGSQL] - update  failed-[%s]",
        breakType,
        err
      );
      jsonString = messageFormatter.FormatMessage(
...
function DeleteBreakType(tenantId, companyId, breakType, callback) {
  ...
  DbConn.ResResourceBreakTypes.destroy({
    where: [
      {
        TenantId: tenantId,
      },
    ...
    .then(function (bType) {
      ...
      logger.info(
        "[DVP-ResResource.DeleteBreakType] - [PGSQL] - delete successfully. [%s] ",
        jsonString
      );
      callback.end(jsonString);
    })
    .error(function (err) {
      logger.error(
        "[DVP-ResResource.DeleteBreakType] - [%s] - [PGSQL] - delete  failed-[%s]",
        breakType,
        err
      );
      jsonString = messageFormatter.FormatMessage(
...
function GetAllBreakTypes(tenantId, companyId, callback) {
  ...
  DbConn.ResResourceBreakTypes.findAll({
    ...
    .then(function (breakObjects) {
      if (breakObjects) {
        logger.info(
          "[DVP-ResResource.GetAllBreakTypes] - [%s] - [PGSQL]  - Data found  - %s-[%s]",
          tenantId,
          companyId,
          JSON.stringify(breakObjects)
        );
        jsonString = messageFormatter.FormatMessage(
          undefined,
          "SUCCESS",
        ...
        logger.error(
          "[DVP-ResResource.GetAllBreakTypes] - [PGSQL]  - No record found for %s - %s  ",
          tenantId,
          companyId
        );
        jsonString = messageFormatter.FormatMessage(
    ...
    .error(function (err) {
      logger.error(
        "[DVP-ResResource.GetAllBreakTypes] - [%s] - [%s] - [PGSQL]  - Error in searching.-[%s]",
        tenantId,
        companyId,
        err
      );
...
function GetAllActiveBreakTypes(tenantId, companyId, callback) {
  ...
  DbConn.ResResourceBreakTypes.findAll({
    ...
    .then(function (breakObjects) {
      if (breakObjects) {
        logger.info(
          "[DVP-ResResource.GetAllActiveBreakTypes] - [%s] - [PGSQL]  - Data found  - %s-[%s]",
          tenantId,
          companyId,
          JSON.stringify(breakObjects)
        );
        ...
        logger.error(
          "[DVP-ResResource.GetAllActiveBreakTypes] - [PGSQL]  - No record found for %s - %s  ",
          tenantId,
          companyId
        );
        jsonString = messageFormatter.FormatMessage(
    ...
    .error(function (err) {
      logger.error(
        "[DVP-ResResource.GetAllActiveBreakTypes] - [%s] - [%s] - [PGSQL]  - Error in searching.-[%s]",
        tenantId,
        companyId,
        err
      );
